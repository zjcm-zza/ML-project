...
class MCTS:
    ...
    @staticmethod
    def performSimulation(state: State, block: np.ndarray):
        ...
        while len(legal_actions) > 0:
            ...
            for action in legal_actions:
                ...
                for _ in range(self.nRoll):
                    depths.append(MCTS.performSimulation(act_state, act_blocks))
            ...
            #print(f"Size: {total_size}, Percent: {size_percent}")  # 调试信息
            legal_actions = self.state.get_legal_Actions(self.block)
            #print(len(legal_actions))

        utilization = total_size / np.prod(self.state.board.shape)
        #print(f"Utilization: {utilization}")  # 调试信息
        return action_list, utilization


    
    # this part is for later improvement
